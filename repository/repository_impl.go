...
func NewRepositoryImpl(elastic *elasticsearch.Client) Repository {
	return &RepositoryImpl{elastic: elastic}
}
...
func (repo *RepositoryImpl) Login(ctx context.Context, db *sql.DB, username, password string) (*domain.User, error) {
	query := "SELECT username, password from users WHERE username = ?"
	result := db.QueryRowContext(ctx, query, username)
	var user domain.User
	if err := result.Scan(&user.Username, &user.Password); err != nil {
		return nil, helper.NewError("user tidak ditemukan")
	}
	return &user, nil
}
...
func (repo *RepositoryImpl) Register(ctx context.Context, db *sql.DB, entity *domain.User) (*domain.User, error) {
	...
	if err != nil {
		return nil, helper.NewError("registrasi gagal")
	}
	...
	doc := map[string]interface{}{"id": entity.Id, "username": entity.Username}
	body, err := json.Marshal(doc)
	if err != nil {
		return nil, helper.NewError("gagal menyiapkan data pengguna")
	}
	docID := fmt.Sprint(entity.Username)
	res, err := repo.elastic.Index("user_cart", strings.NewReader(string(body)), repo.elastic.Index.WithDocumentID(docID), repo.elastic.Index.WithContext(ctx))
	if err != nil {
		return nil, helper.NewError("gagal membuat cart pengguna")
	}
	defer res.Body.Close()
	if res.IsError() {
		return nil, helper.NewError("gagal menyimpan cart pengguna")
	}

	return &domain.User{Id: entity.Id, Username: entity.Username}, nil
}
...
func (repo *RepositoryImpl) AddToCart(ctx context.Context, username string, product *domain.Products, quantity int, db *sql.DB) error {
	...
	if err != nil {
		return helper.NewError("produk tidak ditemukan")
	}

	getRes, err := repo.elastic.Get("user_cart", username)
	if err != nil {
		return helper.NewError("gagal mengambil data cart")
	}
	...
	if getRes.StatusCode != 404 {
		...
		if err := json.NewDecoder(getRes.Body).Decode(&cartData); err != nil {
			return helper.NewError("gagal membaca data cart")
		}
		currentQty := 0
		...
		if currentQty+quantity > productStock {
			return helper.NewError("melebihi stok produk")
		}
	}
	...
	script := `if (ctx._source.cart == null) {ctx._source.cart = [params.product];} else {def item = ctx._source.cart.find(p -> p.product_id == params.product.product_id); if (item != null) {item.quantity += params.quantity;} else {ctx._source.cart.add(params.product);}}`
	...
	if err != nil {
		return helper.NewError("gagal menyiapkan data pembaruan cart")
	}

	res, err := repo.elastic.Update("user_cart", username, bytes.NewReader(updateBody), repo.elastic.Update.WithContext(ctx), repo.elastic.Update.WithRefresh("true"))
	if err != nil {
		return helper.NewError("gagal memperbarui cart")
	}
	defer res.Body.Close()
	if res.IsError() {
		return helper.NewError("gagal memperbarui cart")
...
func (repo *RepositoryImpl) GetCart(ctx context.Context, username string) ([]*domain.CartItem, error) {
	...
	if err := json.NewEncoder(&buf).Encode(query); err != nil {
		return nil, helper.NewError("gagal menyiapkan permintaan pencarian")
	}

	res, err := repo.elastic.Search(repo.elastic.Search.WithContext(ctx), repo.elastic.Search.WithIndex("user_cart"), repo.elastic.Search.WithBody(&buf), repo.elastic.Search.WithTrackTotalHits(true))
	if err != nil {
		return nil, helper.NewError("pencarian cart gagal")
	}
	defer res.Body.Close()
	if res.IsError() {
		return nil, helper.NewError("pencarian cart gagal")
	...
	if err := json.NewDecoder(res.Body).Decode(&esRes); err != nil {
		return nil, helper.NewError("gagal membaca hasil pencarian")
	}
...
func (repo *RepositoryImpl) DeleteCartItem(ctx context.Context, username, productID string) error {
	updateScript := map[string]interface{}{
		"script": map[string]interface{}{
			"source": `if (ctx._source.cart != null) {ctx._source.cart.removeIf(item -> item.product_id == params.product_id);}`,
			"lang":   "painless",
			"params": map[string]interface{}{
	...
	if err := json.NewEncoder(&buf).Encode(updateScript); err != nil {
		return helper.NewError("gagal menyiapkan data penghapusan")
	}

	res, err := repo.elastic.Update("user_cart", username, &buf, repo.elastic.Update.WithContext(ctx), repo.elastic.Update.WithRefresh("true"))
	if err != nil {
		return helper.NewError("gagal menghapus item dari cart")
	}
	defer res.Body.Close()
	if res.IsError() {
		return helper.NewError("gagal menghapus item dari cart")
...
func (repo *RepositoryImpl) DeleteCartItemByQuantity(ctx context.Context, username, productId string, quantity int) error {
	res, err := repo.elastic.Get("user_cart", username, repo.elastic.Get.WithContext(ctx))
	if err != nil {
		return helper.NewError("gagal mengambil data cart")
	}
	defer res.Body.Close()
	if res.StatusCode == 404 {
		return helper.NewError("cart tidak ditemukan")
	}
	var data map[string]interface{}
	if err := json.NewDecoder(res.Body).Decode(&data); err != nil {
		return helper.NewError("gagal membaca cart")
	}
	...
	if err != nil {
		return helper.NewError("gagal menyiapkan data cart terbaru")
	}

	indexRes, err := repo.elastic.Index("user_cart", strings.NewReader(string(body)), repo.elastic.Index.WithDocumentID(username), repo.elastic.Index.WithContext(ctx), repo.elastic.Index.WithRefresh("true"))
	if err != nil {
		return helper.NewError("gagal memperbarui cart")
	}
